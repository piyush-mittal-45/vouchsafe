...
// These form a single wired chain: Gate -> Vault -> Registry (all deployed
// from the current contract source and initialized together).
export const REGISTRY_ID = process.env.NEXT_PUBLIC_REGISTRY_ID || 'CBML5ICVVEU4QWWWAPHMNF3RG2QSP7YENUKUTW5H6KBQ2T6ZRFLO3NUE';
export const VAULT_ID = process.env.NEXT_PUBLIC_VAULT_ID || 'CCMNCGRMMBWSSAPMJT6SOBRKYEYL5VHISP3PMOLT6QDTHAALM42JS4IQ';
export const GATE_ID = process.env.NEXT_PUBLIC_ACCESS_ID || 'CBCFYEZT4VXKIRIVDA2LEVMFGRQMTBK4COCRFSRRLJSYCQ3DBR3WXN4N';
...
// NOTE: Issuer (endorser) authorization is an ADMIN-only operation — the
// VouchRegistry.authorize_endorser method requires the registry admin's
// signature. A static frontend cannot hold the admin key securely, so this is
// intentionally NOT performed client-side. Authorization is granted off-app by
// the registry admin (e.g. via `stellar contract invoke ... authorize_endorser`
// after an off-chain KYC/vetting process). See README "Wallet Connection" and
// "Setup Instructions" for the exact command.
